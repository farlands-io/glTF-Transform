import fs, { access, constants, readFile, writeFile } from "node:fs/promises";
import { stripVTControlCharacters, styleText } from "node:util";
import { BufferUtils, Document, FileUtils, Format, ImageUtils, NodeIO, PropertyType, TextureChannel, TextureInfo, Verbosity, VertexLayout, uuid } from "@gltf-transform/core";
import { DRACO_DEFAULTS, INSTANCE_DEFAULTS, JOIN_DEFAULTS, MESHOPT_DEFAULTS, PALETTE_DEFAULTS, PRUNE_DEFAULTS, QUANTIZE_DEFAULTS, SIMPLIFY_DEFAULTS, TEXTURE_COMPRESS_SUPPORTED_FORMATS, TextureResizeFilter, UNWRAP_DEFAULTS, center, createTransform, dedup, dequantize, draco, fitPowerOfTwo, fitWithin, flatten, getTextureChannelMask, getTextureColorSpace, inspect, instance, join, listTextureInfo, listTextureSlots, mergeDocuments, meshopt, metalRough, palette, partition, prune, quantize, reorder, resample, sequence, simplify, sparse, tangents, textureCompress, unlit, unpartition, unweld, unwrap, weld } from "@gltf-transform/functions";
...
import { ALL_EXTENSIONS, KHRTextureBasisu, KHRXMP } from "@gltf-transform/extensions";
import draco3d from "draco3dgltf";
import path, { join as join$1, resolve } from "path";
...
import os from "os";
import tmp from "tmp";
import { execSync, spawn as spawn$1 } from "node:child_process";
...
function validateConfig(config) {
	for (const extension of config.extensions || []) if (!extension.EXTENSION_NAME) throw new Error("Invalid extension in config.extensions.");
	return config;
}
...
let TableFormat = /* @__PURE__ */ function(TableFormat) {
...
async function formatTable(format, head, rows) {
	switch (format) {
		...
		case TableFormat.CSV: return new Promise((resolve, reject) => {
...
let log = console.log;
function mockConsoleLog(_log) {
	log = _log;
}
//#endregion
//#region src/inspect.ts
async function inspect$1(jsonDoc, io, logger, format) {
	...
	log(await formatTable(format, ["key", "value"], [
	...
	if (rootPacket && rootPacket.listProperties().length > 0) {
		...
		log(await formatTable(format, ["key", "value"], rootPacket.listProperties().map((name) => [name, formatXMP(rootPacket.getProperty(name))])) + "\n\n");
	}
...
async function reportSection(type, format, logger, section) {
	...
	if (section.warnings) section.warnings.forEach((warning) => logger.warn(formatParagraph(warning)));
	log("\n");
}
function formatPropertyReport(property, index, format) {
	...
	for (const key in property) {
		...
		else if (key.match(/size/i) && format !== TableFormat.CSV) row[key] = value > 0 ? formatBytes(value) : "";
		else if (typeof value === "number") row[key] = format !== TableFormat.CSV ? formatLong(value) : value;
		else if (typeof value === "boolean") row[key] = value ? "✓" : "";
		else row[key] = value;
	}
	return row;
}
function getFootnotes(type, header) {
	const footnotes = [];
	if (type === "scenes") {
	...
	if (type === "meshes") {
		for (let i = 0; i < header.length; i++) if (header[i] === "size") header[i] += "¹";
	...
	if (type === "textures") {
		for (let i = 0; i < header.length; i++) if (header[i] === "gpuSize") header[i] += "⁵";
...
var CommandImpl = class {
	...
	action(fn) {
		this._ctx.action(async (args) => {
			const logger = new Logger$1(args.logger);
			return fn({
				...args,
				logger
			});
	...
	constructor(logger) {
		...
		switch (logger.level) {
			...
			case "error":
				...
				break;
			default: this._verbosity = Verbosity.DEBUG;
		}
	}
	getVerbosity() {
	...
	setVerbosity(verbosity) {
		switch (verbosity) {
			...
			case Verbosity.DEBUG:
				...
				break;
			default: throw new Error(`Unexpected verbosity, "${verbosity}".`);
		}
...
var Session = class Session {
	...
	async transform(...transforms) {
		...
		for (const extensionName of ["KHR_draco_mesh_compression", "EXT_meshopt_compression"]) if (document.hasExtension(extensionName)) {
			document.disposeExtension(extensionName);
			this._logger.warn(`Decoded ${extensionName}. Further compression will be lossy.`);
		}
		if (this._display) {
			const tasks = [];
			for (const transform of transforms) tasks.push({
		...
		} else await document.transform(...transforms);
		await document.transform(updateMetadata);
		if (this._outputFormat === Format.GLB) await document.transform(unpartition());
...
async function pLimit(items, limit, fn) {
	...
	await Promise.all([...Array(limit)].map(async () => {
		while (items.length > 0) await fn(items.pop(), index++);
	}));
}
//#endregion
//#region src/utils/process.ts
let spawn = spawn$1;
let commandExists = _commandExists;
let waitExit = _waitExit;
function mockSpawn(_spawn) {
	spawn = _spawn;
}
function mockCommandExists(fn) {
	commandExists = fn;
}
function mockWaitExit(_waitExit) {
	waitExit = _waitExit;
}
let TrustedCommand = /* @__PURE__ */ function(TrustedCommand) {
	TrustedCommand["KTX"] = "ktx";
...
async function _commandExists(cmd) {
	...
	if (!await access(cmd, constants.F_OK).then(() => true).catch(() => false)) return !!execSync(`command -v ${cmd} 2>/dev/null && { echo >&1 ${cmd}; exit 0; }`, { encoding: "utf8" });
	return access(cmd, constants.F_OK | constants.X_OK).then(() => true).catch(() => false);
}
async function _waitExit(process) {
	let stdout = "";
	if (process.stdout) for await (const chunk of process.stdout) stdout += chunk;
	let stderr = "";
	if (process.stderr) for await (const chunk of process.stderr) stderr += chunk;
	return [
...
//#endregion
//#region src/transforms/toktx.ts
...
const toktx = function(options) {
	...
	return createTransform(options.mode, async (doc) => {
		...
		await pLimit(textures, options.jobs, async (texture, textureIndex) => {
			...
			const prefix = `ktx:texture(${texture.getURI() || texture.getName() || `${textureIndex + 1}/${doc.getRoot().listTextures().length}`})`;
			...
			const srcPath = join$1(batchDir.name, `${batchPrefix}_${textureIndex}.${srcExtension}`);
			const dstPath = join$1(batchDir.name, `${batchPrefix}_${textureIndex}.ktx2`);
			...
			if (status !== 0) logger.error(`${prefix}: Failed → \n\n${stderr.toString()}`);
			else {
				texture.setImage(await fs$1.readFile(dstPath)).setMimeType("image/ktx2");
				if (texture.getURI()) texture.setURI(FileUtils.basename(texture.getURI()) + ".ktx2");
				if (!options.mipmaps) for (const info of listTextureInfo(texture)) {
					const minFilter = info.getMinFilter();
					if (minFilter === NEAREST_MIPMAP_LINEAR || minFilter === NEAREST_MIPMAP_NEAREST) info.setMinFilter(NEAREST);
					else if (minFilter === LINEAR_MIPMAP_NEAREST) info.setMinFilter(LINEAR);
				}
			}
			const dstBytes = texture.getImage().byteLength;
			logger.debug(`${prefix}: ${formatBytes(srcBytes)} → ${formatBytes(dstBytes)} bytes`);
		});
		if (options.cleanup) await rm(batchDir.name, { recursive: true });
		if (!doc.getRoot().listTextures().some((t) => t.getMimeType() === "image/ktx2")) basisuExtension.dispose();
	});
};
/**********************************************************************************************
* Utilities.
...
function createParams(texture, slots, channels, numTextures, options) {
	const colorSpace = getTextureColorSpace(texture);
	const params = [];
	if (options.mipmaps) params.push("--generate-mipmap");
	if (options.mipmaps && options.filter !== GLOBAL_DEFAULTS.filter) params.push("--mipmap-filter", options.filter);
	if (options.mipmaps && options.filterScale !== GLOBAL_DEFAULTS.filterScale) params.push("--mipmap-filter-scale", options.filterScale);
	...
	if (options.mode === Mode.UASTC) {
		...
		if (_options.rdo && !isNormalMap) {
			...
			if (_options.rdoLambda !== UASTC_DEFAULTS.rdoLambda) params.push("--uastc-rdo-l", _options.rdoLambda);
			if (_options.rdoDictionarySize !== UASTC_DEFAULTS.rdoDictionarySize) params.push("--uastc-rdo-d", _options.rdoDictionarySize);
			if (_options.rdoBlockScale !== UASTC_DEFAULTS.rdoBlockScale) params.push("--uastc-rdo-b", _options.rdoBlockScale);
			if (_options.rdoStdDev !== UASTC_DEFAULTS.rdoStdDev) params.push("--uastc-rdo-s", _options.rdoStdDev);
			if (!_options.rdoMultithreading) params.push("--uastc-rdo-m");
		}
		if (_options.zstd && _options.zstd > 0) params.push("--zstd", _options.zstd);
	} else {
		const _options = options;
		params.push("--encode", "basis-lz");
		if (_options.quality !== ETC1S_DEFAULTS.quality) params.push("--qlevel", _options.quality);
		if (_options.compression !== ETC1S_DEFAULTS.compression) params.push("--clevel", _options.compression);
		if (_options.rdo && !isNormalMap) {
			if (_options.maxEndpoints !== ETC1S_DEFAULTS.maxEndpoints) params.push("--max-endpoints", _options.maxEndpoints);
			if (_options.maxSelectors !== ETC1S_DEFAULTS.maxSelectors) params.push("--max-selectors", _options.maxSelectors);
		...
		} else params.push("--no-endpoint-rdo", "--no-selector-rdo");
	}
	if (colorSpace === "srgb") params.push("--assign-oetf", "srgb", "--assign-primaries", "bt709");
	else if (colorSpace === "srgb-linear") params.push("--assign-oetf", "linear", "--assign-primaries", "bt709");
	else if (slots.length && !colorSpace) params.push("--assign-oetf", "linear", "--assign-primaries", "none");
	if (channels === R) params.push("--format", "R8_UNORM");
	else if (channels === G || channels === (R | G)) params.push("--format", "R8G8_UNORM");
	else if (!(channels & A)) params.push("--format", colorSpace === "srgb" ? "R8G8B8_SRGB" : "R8G8B8_UNORM");
	else params.push("--format", colorSpace === "srgb" ? "R8G8B8A8_SRGB" : "R8G8B8A8_UNORM");
...
async function checkKTXSoftware(logger) {
	if (!await commandExists(TrustedCommand.KTX) && !process.env.CI) throw new Error(`Command "ktx" not found. Please install KTX-Software ${KTX_SOFTWARE_VERSION_MIN}+, from:
...
https://github.com/KhronosGroup/KTX-Software`);
	...
	if (status !== 0 || !version) throw new Error(`Unable to find "ktx" version. Confirm KTX-Software ${KTX_SOFTWARE_VERSION_MIN}+ is installed.`);
	else logger.debug(`ktx: Found KTX-Software ${version}.`);
	return version;
}
...
//#endregion
//#region src/transforms/ktxdecompress.ts
const KTX_DECOMPRESS_DEFAULTS = {
	jobs: 2 * (os.cpus().length || 1),
	cleanup: true
};
const ktxdecompress = function(options = KTX_DECOMPRESS_DEFAULTS) {
	options = {
		...KTX_DECOMPRESS_DEFAULTS,
		...options
	};
	return createTransform("ktxdecompress", async (doc) => {
		const logger = doc.getLogger();
		await checkKTXSoftware(logger);
		const batchPrefix = uuid();
		const batchDir = tmp.dirSync({ prefix: "gltf-transform-" });
		const basisuExtension = doc.createExtension(KHRTextureBasisu);
		await pLimit(doc.getRoot().listTextures(), options.jobs, async (texture, textureIndex) => {
			const prefix = `ktx:texture(${texture.getURI() || texture.getName() || `${textureIndex + 1}/${doc.getRoot().listTextures().length}`})`;
			const srcMimeType = texture.getMimeType();
			if (srcMimeType !== "image/ktx2") return;
			const srcImage = texture.getImage();
			const srcExtension = texture.getURI() ? FileUtils.extension(texture.getURI()) : ImageUtils.mimeTypeToExtension(srcMimeType);
			const srcSize = texture.getSize();
			const srcBytes = srcImage ? srcImage.byteLength : null;
			if (!srcImage || !srcSize || !srcBytes) {
				logger.warn(`${prefix}: Skipping, unreadable texture.`);
			...
			const srcPath = join$1(batchDir.name, `${batchPrefix}_${textureIndex}.${srcExtension}`);
			const dstPath = join$1(batchDir.name, `${batchPrefix}_${textureIndex}.png`);
			await fs$1.writeFile(srcPath, srcImage);
			const [status, _stdout, stderr] = await waitExit(spawn("ktx", [
				"extract",
				srcPath,
				dstPath
			]));
			if (status !== 0) logger.error(`${prefix}: Failed → \n\n${stderr.toString()}`);
				...
				texture.setImage(await fs$1.readFile(dstPath)).setMimeType("image/png");
				if (texture.getURI()) texture.setURI(FileUtils.basename(texture.getURI()) + ".png");
			}
			const dstBytes = texture.getImage().byteLength;
			logger.debug(`${prefix}: ${formatBytes(srcBytes)} → ${formatBytes(dstBytes)} bytes`);
		});
...
//#endregion
//#region src/transforms/ktxfix.ts
const NAME$1 = "ktxfix";
function ktxfix() {
	return async (doc) => {
		const logger = doc.getLogger();
		...
		for (const texture of doc.getRoot().listTextures()) {
			...
			if (listTextureSlots(texture).length === 0) continue;
			const colorPrimaries = getTextureColorSpace(texture) === "srgb" ? KHR_DF_PRIMARIES_BT709 : KHR_DF_PRIMARIES_UNSPECIFIED;
...
//#endregion
//#region src/transforms/merge.ts
const NAME = "merge";
const merge = (options) => {
	...
	return async (document) => {
		...
		for (let i = 0; i < paths.length; i++) {
			...
			if ([
				...
				"webp",
				"ktx2"
			].includes(extension)) document.createTexture(basename).setImage(fs$2.readFileSync(path)).setMimeType(ImageUtils.extensionToMimeType(extension)).setURI(basename + "." + extension);
			else if (["gltf", "glb"].includes(extension)) mergeDocuments(document, renameScenes(basename, await io.read(path)));
		...
		if (!options.partition) await document.transform(unpartition());
		logger.debug(`${NAME}: Complete.`);
	};
};
function renameScenes(name, document) {
	const scenes = document.getRoot().listScenes();
	for (let i = 0; i < scenes.length; i++) if (!scenes[i].getName()) scenes[i].setName(name + (scenes.length > 1 ? ` (${i + 1}/${scenes.length})` : ""));
	return document;
}
...
function createUniqueURIs(resources) {
	...
	for (const resource of resources) {
		...
		for (let i = 2; used[uri]; i++) uri = `${prefix}_${i++}.${extension}`;
		resource.setURI(uri);
		used[uri] = true;
	}
}
//#endregion
//#region src/transforms/xmp.ts
const DEFAULT_LANG = "en-US";
const XMP_DEFAULTS = {
	packet: "",
	reset: false
};
var Prompt = /* @__PURE__ */ function(Prompt) {
...
async function* generateQuestions(results) {
	...
	if (prompts.has(Prompt.CREATOR)) yield {
		type: "text",
		name: "dc:creator",
	...
	if (prompts.has(Prompt.TITLE)) yield {
		type: "text",
		name: "dc:title",
	...
	if (prompts.has(Prompt.DESCRIPTION)) yield {
		type: "text",
		name: "dc:description",
	...
	if (prompts.has(Prompt.RELATED)) yield {
		type: "list",
		name: "dc:relation",
	...
	if (prompts.has(Prompt.RIGHTS)) {
		...
		if (results._rights === "UNLICENSED") {
			...
			if (/^https?:\/\//.test(usage)) results["xmpRights:WebStatement"] = usage;
			else if (usage) results["xmpRights:UsageTerms"] = createLanguageAlternative(usage, lang);
		}
		if (results._rights === "OTHER") {
			...
			if (results._isLicenseSPDX) yield {
				type: "text",
	...
	if (prompts.has(Prompt.CREATE_DATE)) yield {
		type: "date",
		name: "xmp:CreateDate",
	...
	if (prompts.has(Prompt.PREFERRED_SURFACE)) yield {
		type: "multiselect",
...
const xmp = (_options = XMP_DEFAULTS) => {
	...
	return async (document) => {
		...
		try {
			for await (const question of generateQuestions(results)) Object.assign(results, await prompts(question));
		} catch (e) {
			checkTTY(e, logger);
			throw e;
		}
		...
		let numProperties = 0;
		for (const name in results) if (!name.startsWith("_") && !name.startsWith("@") && results[name]) {
			packet.setProperty(name, results[name]);
			numProperties++;
		}
		if (numProperties === 0) throw new Error("xmp: No properties added.");
		packet.setProperty("xmp:MetadataDate", (/* @__PURE__ */ new Date()).toISOString().substring(0, 10));
		root.setExtension("KHR_xmp_json_ld", packet);
...
function validatePacket(packetDef) {
	...
	if (!context) throw new Error("Missing @context in packet.");
	for (const name in packetDef) {
		if (name.startsWith("@")) continue;
		const prefix = name.split(":")[0];
		if (!prefix) throw new Error(`Invalid property, "${name}"`);
		if (!(prefix in context)) throw new Error(`Missing context definition, "${prefix}"`);
	}
	return packetDef;
}
/**
...
function checkTTY(error, logger) {
	if (error.isTtyError) logger.warn("Unable to run \"inquirer\" session in this terminal environment. Try another terminal or provide a --packet JSON-LD input.");
}
...
//#endregion
//#region src/utils/gzip.ts
async function gzip$1(src) {
...
async function validate(input, options, logger) {
	...
	return validator.validateBytes(new Uint8Array(buffer), {
		...
		if (options.format === TableFormat.CSV) await printCSV(report);
		else {
		...
		if (report.issues.messages.some((message) => message.severity === 0)) throw new Error("Validation detected errors.");
	});
}
...
async function printTable(header, severity, report, logger, format) {
	...
	if (messages.length) log(await formatTable(format, HEADER, messages.map(Object.values)));
	else logger.info(`No ${header}s found.`);
	log("\n");
}
//#endregion
//#region src/cli.ts
...
const programReady = new Promise((resolve) => {
	if (process.argv.includes("--config")) loadConfig(process.argv[process.argv.indexOf("--config") + 1]);
...
program.version("4.4.0-alpha.1").description("Command-line interface (CLI) for the glTF Transform SDK.");
if (process.argv && !process.argv.includes("--no-editorial")) program.help(`
...
`.trim(), { sectionName: "COMMERCIAL USE" });
...
Use --format=csv or --format=md for alternative display formats.
	...
	io.setLogger(logger);
	await inspect$1(await io.readAsJSON(args.input), io, logger, options.format);
});
...
`.trim()).argument("<input>", INPUT_DESC).argument("<output>", OUTPUT_DESC).action(({ args, logger }) => Session.create(io, logger, args.input, args.output).transform());
...
commands or using the scripting API.
	...
	if (opts.simplify) transforms.push(simplify({
	...
	if (opts.textureCompress === "ktx2") {
		...
		transforms.push(textureCompress({
			encoder,
			resize: [opts.textureSize, opts.textureSize],
			targetFormat: opts.textureCompress === "auto" ? void 0 : opts.textureCompress,
			limitInputPixels: options.limitInputPixels
		}));
	}
	if (opts.compress === "draco") {
		if (opts.weld === false) logger.warn("Ignoring --no-weld, required for Draco compression.");
		transforms.push(draco());
	} else if (opts.compress === "meshopt") transforms.push(meshopt({
		encoder: MeshoptEncoder,
		level: opts.meshoptLevel
	}));
	else if (opts.compress === "quantize") transforms.push(quantize());
...
resources as needed. Partitioning is supported only for .gltf, not .glb, files.
	...
	validator: Validator.BOOLEAN,
	default: false
}).action(({ args, options, logger }) => Session.create(io, logger, args.input, args.output).transform(partition(options)));
...
}).action(({ args, options, logger }) => Session.create(io, logger, args.input, args.output).transform(prune(options)));
...
`).argument("<input>", INPUT_DESC).action(async ({ args, logger }) => {
	const inBuffer = await readFile(args.input);
	const outBuffer = await gzip$1(inBuffer);
...
`).argument("<input>", INPUT_DESC).argument("<output>", OUTPUT_DESC).option("--packet <path>", "Path to XMP packet (.jsonld or .json)").option("--reset", "Reset metadata and remove XMP extension", {
	validator: Validator.BOOLEAN,
	default: false
}).action(async ({ args, options, logger }) => Session.create(io, logger, args.input, args.output).transform(xmp({ ...options })));
...
attached a surface, like a ceiling fan, the pivot may be located above instead.
	...
	default: "center"
}).action(({ args, options, logger }) => Session.create(io, logger, args.input, args.output).transform(center({ ...options })));
...
https://github.com/KhronosGroup/glTF/tree/master/extensions/2.0/Vendor/EXT_mesh_gpu_instancing.
	...
	validator: Validator.NUMBER,
	default: INSTANCE_DEFAULTS.min
}).action(({ args, options, logger }) => Session.create(io, logger, args.input, args.output).transform(instance({ ...options })));
...
moved.
	`.trim()).argument("<input>", INPUT_DESC).argument("<output>", OUTPUT_DESC).action(({ args, options, logger }) => Session.create(io, logger, args.input, args.output).transform(flatten({ ...options })));
...
EXT_mesh_gpu_instancing.
	...
	validator: Validator.BOOLEAN,
	default: JOIN_DEFAULTS.keepNamed
}).action(({ args, options, logger }) => Session.create(io, logger, args.input, args.output).transform(dedup({ propertyTypes: [PropertyType.MATERIAL] }), flatten(), join({ ...options })));
...
}).action(({ args, options, logger }) => Session.create(io, logger, args.input, args.output).transform(draco(options)));
...
}).action(({ args, options, logger }) => Session.create(io, logger, args.input, args.output).transform(meshopt({
	encoder: MeshoptEncoder,
	...options
})));
...
can be reduced, and the GPU uses the vertex cache more efficiently.
	`.trim()).argument("<input>", INPUT_DESC).argument("<output>", OUTPUT_DESC).action(({ args, options, logger }) => Session.create(io, logger, args.input, args.output).transform(weld(options)));
...
particular software application.
	`.trim()).argument("<input>", INPUT_DESC).argument("<output>", OUTPUT_DESC).action(({ args, options, logger }) => Session.create(io, logger, args.input, args.output).transform(unweld(options)));
...
compute MikkTSpace tangents at runtime.
	...
	validator: Validator.BOOLEAN,
	default: false
}).action(({ args, options, logger }) => Session.create(io, logger, args.input, args.output).transform(unweld(), tangents({
	generateTangents: mikktspace.generateTangents,
	...options
}), weld()));
program.command("unwrap", "Generate texcoords").help(`
...
coordinates suitable for baking lightmaps or texture painting.
	`.trim()).argument("<input>", INPUT_DESC).argument("<output>", OUTPUT_DESC).option("--texcoord <index>", "Target texture coordinate index. 0 = TEXCOORD_0, etc.", {
	validator: Validator.NUMBER,
	default: 0
...
}).action(({ args, options, logger }) => Session.create(io, logger, args.input, args.output).transform(unwrap({
	watlas,
	...options
})));
...
Based on the meshoptimizer library (https://github.com/zeux/meshoptimizer).
	...
	default: "size"
}).action(({ args, options, logger }) => Session.create(io, logger, args.input, args.output).transform(reorder({
	encoder: MeshoptEncoder,
	...options
})));
...
`.trim()).argument("<input>", INPUT_DESC).argument("<output>", OUTPUT_DESC).option("--ratio <ratio>", "Target ratio (0–1) of vertices to keep", {
	...
	validator: Validator.BOOLEAN,
	default: SIMPLIFY_DEFAULTS.lockBorder
}).action(async ({ args, options, logger }) => Session.create(io, logger, args.input, args.output).transform(simplify({
	simplifier: MeshoptSimplifier,
	...options
})));
...
should be used as input, and then compressed after this conversion.
	`.trim()).argument("<input>", INPUT_DESC).argument("<output>", OUTPUT_DESC).action(({ args, logger }) => Session.create(io, logger, args.input, args.output).transform(metalRough()));
...
Unlit materials are also helpful for non-physically-based visual styles.
	`.trim()).argument("<input>", INPUT_DESC).argument("<output>", OUTPUT_DESC).action(({ args, logger }) => Session.create(io, logger, args.input, args.output).transform(unlit()));
...
`.trim()).argument("<input>", INPUT_DESC).argument("<output>", OUTPUT_DESC).option("--pattern <pattern>", "Pattern (glob) to match textures, by name or URI.", { validator: Validator.STRING }).option("--filter", "Resampling filter", {
	validator: [TextureResizeFilter.LANCZOS3, TextureResizeFilter.LANCZOS2],
	default: TextureResizeFilter.LANCZOS3
}).option("--width <pixels>", "Maximum width (px) of output textures.", { validator: Validator.NUMBER }).option("--height <pixels>", "Maximum height (px) of output textures.", { validator: Validator.NUMBER }).option("--power-of-two <pot>", "Resize in two dimensions. Overrides --width and --height.", { validator: [
	"nearest",
	"ceil",
	"floor"
] }).action(async ({ args, options, logger }) => {
	...
	if (options.powerOfTwo) {
		if (options.width || options.height) logger.warn("When --power-of-two is specified, --width and --height are ignored.");
		resize = `${options.powerOfTwo}-pot`;
	} else if (Number.isInteger(options.width) && Number.isInteger(options.height)) resize = [Number(options.width), Number(options.height)];
	else throw new Error(`Must specify --width and --height, or --power-of-two.`);
...
normal maps and ETC1S for other textures, for example.`.trim())).argument("<input>", INPUT_DESC).argument("<output>", OUTPUT_DESC).option("--pattern <pattern>", "Pattern (glob) to match textures, by name or URI.", { validator: Validator.STRING }).option("--slots <slots>", "Texture slots to include (glob)", { validator: Validator.STRING }).option("--mipmaps <bool>", "Generate mipmaps.", {
	validator: Validator.BOOLEAN,
...
for textures where the quality is sufficient.`.trim())).argument("<input>", INPUT_DESC).argument("<output>", OUTPUT_DESC).option("--pattern <pattern>", "Pattern (glob) to match textures, by name or URI.", { validator: Validator.STRING }).option("--slots <slots>", "Texture slots to include (glob)", { validator: Validator.STRING }).option("--mipmaps <bool>", "Generate mipmaps.", {
	validator: Validator.BOOLEAN,
...
program.command("ktxdecompress", "KTX + Basis texture decompression").help(`
		Decompresses KTX2 textures in KTX2 format, converting to PNG.
		Intended for debugging, or to resolve compatibility issues in
		software that doesn't support KTX2 textures.`.trim()).argument("<input>", INPUT_DESC).argument("<output>", OUTPUT_DESC).action(({ args, logger }) => Session.create(io, logger, args.input, args.output).transform(ktxdecompress()));
...
affects only the container metadata.`.trim()).argument("<input>", INPUT_DESC).argument("<output>", OUTPUT_DESC).action(({ args, logger }) => Session.create(io, logger, args.input, args.output).transform(ktxfix()));
...
`.trim();
program.command("avif", "AVIF texture compression").help(TEXTURE_COMPRESS_SUMMARY.replace(/{VARIANT}/g, "AVIF")).argument("<input>", INPUT_DESC).argument("<output>", OUTPUT_DESC).option("--pattern <pattern>", "Pattern (glob) to match textures, by name or URI.", { validator: Validator.STRING }).option("--formats <formats>", "Texture formats to include", {
	validator: [...TEXTURE_COMPRESS_SUPPORTED_FORMATS, "*"],
	default: "*"
}).option("--slots <slots>", "Texture slots to include (glob)", {
	validator: Validator.STRING,
	default: "*"
}).option("--quality <quality>", "Quality, 1-100", { validator: Validator.NUMBER }).option("--effort <effort>", "Level of CPU effort to reduce file size, 0-100", { validator: Validator.NUMBER }).option("--lossless <lossless>", "Use lossless compression mode", {
	validator: Validator.BOOLEAN,
	default: false
}).action(async ({ args, options, logger }) => {
	const pattern = options.pattern ? micromatch.makeRe(String(options.pattern), MICROMATCH_OPTIONS) : null;
...
program.command("webp", "WebP texture compression").help(TEXTURE_COMPRESS_SUMMARY.replace(/{VARIANT}/g, "WebP")).argument("<input>", INPUT_DESC).argument("<output>", OUTPUT_DESC).option("--pattern <pattern>", "Pattern (glob) to match textures, by name or URI.", { validator: Validator.STRING }).option("--formats <formats>", "Texture formats to include", {
	validator: [...TEXTURE_COMPRESS_SUPPORTED_FORMATS, "*"],
	default: "*"
}).option("--slots <slots>", "Texture slots to include (glob)", {
	validator: Validator.STRING,
...
program.command("png", "PNG texture compression").help(TEXTURE_COMPRESS_SUMMARY.replace(/{VARIANT}/g, "PNG")).argument("<input>", INPUT_DESC).argument("<output>", OUTPUT_DESC).option("--pattern <pattern>", "Pattern (glob) to match textures, by name or URI.", { validator: Validator.STRING }).option("--formats <formats>", "Texture formats to include", {
	validator: [...TEXTURE_COMPRESS_SUPPORTED_FORMATS, "*"],
	default: "png"
}).option("--slots <slots>", "Texture slots to include (glob)", {
	validator: Validator.STRING,
	default: "*"
}).option("--quality <quality>", "Quality, 1-100", { validator: Validator.NUMBER }).option("--effort <effort>", "Level of CPU effort to reduce file size, 0-100", { validator: Validator.NUMBER }).action(async ({ args, options, logger }) => {
	const pattern = options.pattern ? micromatch.makeRe(String(options.pattern), MICROMATCH_OPTIONS) : null;
	const formats = regexFromArray([options.formats]);
	const slots = micromatch.makeRe(String(options.slots), MICROMATCH_OPTIONS);
	const { default: encoder } = await import("sharp");
...
program.command("jpeg", "JPEG texture compression").help(TEXTURE_COMPRESS_SUMMARY.replace(/{VARIANT}/g, "JPEG")).argument("<input>", INPUT_DESC).argument("<output>", OUTPUT_DESC).option("--pattern <pattern>", "Pattern (glob) to match textures, by name or URI.", { validator: Validator.STRING }).option("--formats <formats>", "Texture formats to include", {
	validator: [...TEXTURE_COMPRESS_SUPPORTED_FORMATS, "*"],
	default: "jpeg"
}).option("--slots <slots>", "Texture slots to include (glob)", {
	validator: Validator.STRING,
	default: "*"
}).option("--quality <quality>", "Quality, 1-100", { validator: Validator.NUMBER }).action(async ({ args, options, logger }) => {
	const pattern = options.pattern ? micromatch.makeRe(String(options.pattern), MICROMATCH_OPTIONS) : null;
	const formats = regexFromArray([options.formats]);
	const slots = micromatch.makeRe(String(options.slots), MICROMATCH_OPTIONS);
	const { default: encoder } = await import("sharp");
...
--tolerance, and should have no visible effect on animation playback.
	...
	validator: Validator.NUMBER,
	default: 1e-4
}).action(({ args, options, logger }) => Session.create(io, logger, args.input, args.output).transform(resample({
	ready,
	resample: resample$1,
	...options
})));
...
file sizes.
	`.trim()).argument("<input>", INPUT_DESC).argument("<output>", OUTPUT_DESC).action(({ args, options, logger }) => Session.create(io, logger, args.input, args.output).transform(sparse(options)));
...
program.option("--config <path>", "Installs custom commands or extensions. (EXPERIMENTAL)", { validator: Validator.STRING });
...
//#endregion
export { ETC1S_DEFAULTS, Filter, Mode, TableFormat, TrustedCommand, UASTC_DEFAULTS, Validator, XMP_DEFAULTS, _waitExit, checkKTXSoftware, commandExists, formatBytes, formatHeader, formatLong, formatParagraph, formatTable, formatXMP, ktxdecompress, ktxfix, log, merge, mockCommandExists, mockConsoleLog, mockSpawn, mockWaitExit, pLimit, program, programReady, spawn, toktx, waitExit, xmp };
